  ...
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction(async transaction => {
      ...
      duplicateLabels.map(dupeLabel => {
        duplicateLabelTitlesByUserId[dupeLabel.userId] = duplicateLabelTitlesByUserId[dupeLabel.userId] || [];
        if (!duplicateLabelTitlesByUserId[dupeLabel.userId].includes(dupeLabel.title)) {
      ...
      for (let i = 0; i < Object.keys(duplicateLabelTitlesByUserId).length; i++) {
        ...
        for (let j = 0; j < duplicateLabelTitles.length; j++) {
          ...
          const duplicateLabelIds = (await queryInterface.sequelize.query(`
