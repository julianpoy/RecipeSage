...
const convertToISO8601Time = (time) => {
  ...
  const minuteMatch = time.match(/(\d* ?(\d+\/\d+)?(\.\d+)?) *(minutes?|mins?|m)/i);
...
const getLabelsFromSchema = (jsonLD) => {
  ...
  if (typeof recipeCategory === 'string') return recipeCategory.split(',').map(el => el.trim());
  if (typeof recipeCategory[0] === 'string') return recipeCategory.map(el => el.trim());
