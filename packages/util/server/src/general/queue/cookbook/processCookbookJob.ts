...
export const processCookbookJob = async (
  ...
  const stream = await generateCookbookPDFStream(
    recipes,
    {
      ...
      includeLastMade: jobMeta.cookbookIncludeLastMade ?? false,
      includeLinkedRecipes: jobMeta.cookbookIncludeLinkedRecipes ?? false,
      includeRecipeSageUrl: jobMeta.cookbookIncludeRecipeSageUrl ?? false,
      language,
      introductionLabel,
      contentsLabel,
      byAuthorTemplate,
