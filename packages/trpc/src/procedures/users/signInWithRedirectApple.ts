...
export const signInWithRedirectApple = publicProcedure
  .meta({
    openapi: {
      method: "POST",
      path: "/users/signInWithRedirectApple",
      tags: ["users"],
      summary: "Exchange an redirect Apple auth code for a session token",
      description:
  ...
  .mutation(async ({ input }) => {
    const secret = config.apple.signIn.privateKey;
    if (!secret) {
      throw new TRPCError({
