...
import { REDIRECT_GOOGLE_AUTH_CODE_HMAC_PREFIX } from "@recipesage/util/server/general";
...
describe("signInWithRedirectGoogle", () => {
  ...
  describe("success", () => {
    test("returns a session for an existing user even when allowRegistration is false", async () => {
      ...
      const response = await anonymousTrpc.users.signInWithRedirectGoogle({
        code: makeCode({
          email,
          name: "Test",
  ...
  describe("error", () => {
    test("throws when the PKCE verifier does not match the challenge", async () => {
      ...
      await expect(
        anonymousTrpc.users.signInWithRedirectGoogle({
          code: makeCode({
            email,
            name: "Test",
            allowRegistration: true,
