...
export async function lcbImportJobHandler(
  ...
  try {
    ...
    tableMap.t_recipe = (tableMap.t_recipe || []).filter(
      (recipe) =>
        !!recipe.recipeid &&
        (queueItem.options?.includeStockRecipes || !!recipe.modifieddate),
    );
    ...
    for (const lcbRecipe of tableMap.t_recipe || []) {
      ...
      if (queueItem.options?.includeTechniques) {
      ...
      const images: string[] = [];
      if (!queueItem.options?.excludeImages) {
        ...
        if (!queueItem.options?.canImportMultipleImages && images.length > 1) {
          images.splice(1);
        }
      }
