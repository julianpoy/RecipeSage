...
then
    ...
    server=root@db3.recipesage.com
    scp -r $server:/pg-backup/$(ssh $server 'ls -t /pg-backup | head -1') restore.sql.gz
fi
...
gunzip -c restore.sql.gz | docker exec -i "$(docker compose ps -q postgres)" psql -U $POSTGRES_USER -d $POSTGRES_DB
